import { DocumentsDeletionQuery, DocumentsIds, Meilisearch, Settings } from 'meilisearch'
...
export class MeiliSearchService extends SearchUtils.AbstractSearchService {
  ...
  protected getLanguageIndexName(baseKey: string, language?: string): string {
    ...
    if (i18n?.strategy !== 'separate-index' || !language) {
      return baseKey
    }
  ...
  getFieldsForType(type: string) {
    ...
    Object.values(this.config_.settings ?? {})
      .filter((config) => {
        return config.type === type && config.enabled !== false
      })
      .forEach((config) => {
  ...
  getIndexesByType(type: string) {
    ...
    if (i18n?.strategy === 'separate-index') {
      ...
      return baseIndexes.flatMap((baseIndex) => {
        return languages.map((lang) => {
          return this.getLanguageIndexName(baseIndex, lang)
        })
      })
    }
  ...
  async addDocuments(indexName: string, documents: unknown[], _type: string, options?: AddDocumentsOptions) {
    ...
    const records = documents.filter((d): d is Record<string, unknown> => {
      return typeof d === 'object' && d !== null && !Array.isArray(d)
    })

    if (i18n?.strategy === 'separate-index') {
      const langIndexName = this.getLanguageIndexName(indexName, language ?? i18n.defaultLanguage)
      const transformedDocuments = await this.getTransformedDocuments(indexName, records, i18nOptions)
  ...
  async replaceDocuments(indexName: string, documents: unknown[], type: string, options?: AddDocumentsOptions) {
  ...
  private async updateIndexSettings(indexName: string, indexSettings: Settings) {
  ...
  async upsertIndex(indexName: string, settings: Pick<SearchTypes.IndexSettings, 'primaryKey'>) {
    ...
    try {
      ...
      if (
        ...
        error !== null &&
        'code' in error &&
        error.code === meilisearchErrorCodes.INDEX_NOT_FOUND
      ) {
